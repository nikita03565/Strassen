...
def strassenR(A, B, LEAF_SIZE = 64):
    n = len(A)
    if n <= LEAF_SIZE:
...
def start(aFile, bFile, aN, bN, LEAF_SIZE = 64):
    matrixA = read(aFile, aN)
    matrixB = read(bFile, bN)

    print("Strassen:")
    t1 = time.time()
    C = strassen(matrixA, matrixB)
    t2 = time.time()
    return t2 - t1
...
if __name__ == '__main__':
    ...
    for i in range(100, 2200, 100):
        aFileName = "a" + str(i) + ".in"
        bFileName = "b" + str(i) + ".in"
